...
# BBQ Sexual-orientation (n=216), ChatGPT only. Query once per temperature
# {0,.5,1,1.5,2}, then recompute stats on nested prefix subsamples
# {5,10,20,50,80,100}% (no extra calls) to show behaviour vs sample size and
# temperature. t=1.0 reuses Experiment C's cache. main.R `D`.
######################################
...
for (temp in TEMPERATURES_ROBUSTNESS) {
  # dataset_name = "bbq" so t=1.0 reuses Experiment C's cache.
