...
# Loaders reproduce the paper's per-category sizes (CrowS EN/FR, Winogender) and a
# CrowS prompt matches the authors' wording. Fixtures verified against the raw files.
######################################

# Load the library if it isn't already (run from the project root).
if (!exists("load_crows")) source("src/load_all.R")
library(testthat)
